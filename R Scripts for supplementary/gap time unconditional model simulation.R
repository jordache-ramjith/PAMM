...
simdat[[5]]=simulation(ns=500,ne=100,maxtime=20)
#dependence plus low heterogeneity
simdat[[6]]=simulation(ns=500,var=0.2,ne=100,maxtime=20)
#dependence plus moderate heterogeneity
simdat[[7]]=simulation(ns=500,var=0.5,ne=100,maxtime=20)
#dependence + high heterogeneity
simdat[[8]]=simulation(ns=500,var=1,ne=100,maxtime=20)

sumey=lapply(simdat,FUN=function(x){lapply(x, FUN=function(y){sum(y$status[y$event==40])})})
...
for (j in 1:length(simdat)){
  ...
  for(i in 1:length(simdat[[j]])){
    ...
    coxmod1 = coxme(Surv(gap,status)~x+(1|n),data=simdat[[j]][[i]])
...
betadens=lapply(dat,FUN=function(x){ggplot(data=x)+
    ...
    scale_y_continuous(breaks = seq(0,6,1),limits=c(0,6.5),expand=c(0,0))+
...
ggsave("betaplot_unstratified.pdf",betaplot,width = 10, height = 7, dpi = 300,device='pdf')
...
ggsave("sigmaplot_unstratified.pdf",sigmaplot,width = 10, height = 7, dpi = 300,device='pdf')
...
longshortdat<-rbindlist(lapply(1:length(shortdat)
                               , function(x){ setDT(shortdat[[x]])[, id:=x]}), use.names=TRUE, fill=TRUE)
...
save.image("Unstratified_models_WS_box_steffensmeier_de_boef.RData")
