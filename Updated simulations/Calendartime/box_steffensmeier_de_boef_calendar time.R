...
source("C:/Users/manZ679255/Google Drive/phd 2020/Thesis/Paper 3/Analysis/Function for recurrent PAMM/Function for recurrent PAMM/Function.R")
...
#a list for the simulation datasets under all 8 scenarios to be saved in
simdat=list()
...
#no heterogeneity
simdat[[5]]=simulation(ns=500,maxtime = 100)
#low heterogeneity
simdat[[6]]=simulation(ns=500,var=0.2,maxtime = 100)
#moderate heterogeneity
simdat[[7]]=simulation(ns=500,var=0.5,maxtime = 100)
#high heterogeneity
simdat[[8]]=simulation(ns=500,var=1,maxtime = 100)
...
betadens=lapply(dat,FUN=function(x){ggplot(data=x)+
    ...
    #geom_density(aes(x=coef2,fill="CPH-unstratified"),alpha=0.5)+
...
ggsave("betaplot_calendar.pdf",betaplot,width = 10, height = 7, dpi = 300,device='pdf')
...
sigmadens=lapply(dat,FUN=function(x){ggplot(data=x)+
    ...
    #geom_density(aes(x=sigma2,fill="CPH-unstratified"),alpha=0.5)+
...
ggsave("sigmaplot_calendar.pdf",sigmaplot,width = 10, height = 7, dpi = 300,device='pdf')
...
library(data.table)
longshortdat<-rbindlist(lapply(1:length(shortdat), function(x){ setDT(shortdat[[x]])[, id:=x]}) , use.names=TRUE, fill=TRUE)
...
save.image("WS_box_steffensmeier_de_boef_ct.RData")
